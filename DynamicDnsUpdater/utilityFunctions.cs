...
namespace DynamicDnsUpdater
{
    class utilityFunctions
    {
        ...
        const string taskName = "DynamicDnsUpdater by makNET"; //both regedit and schtasks
        ...
        public enum UpdateStatus { OK, NotConnected, Firewalled, UserNotFound, Unauthorized, UpdateFailed, InvalidUpdateLink, InvalidHostname, InvalidCredentials, UnknownError };
        /// <summary>
        /// Updates the DDNS
        /// </summary>
        ...
        public UpdateStatus UpdateDns(string user, string password, string hostname, string updateLink)
        {
            try
            {
                ...
                try
                {
                    webRequest = (HttpWebRequest)WebRequest.Create(new Uri("https://" + updateLink + "/nic/duc?app=windows&hostname=" + hostname));
                }
                ...
                if (success)
                    ...
                    {
                        // Handle error case
                        string error = jsonResponse.Value<string>("error");
                        ...
                        if (!string.IsNullOrEmpty(error))
                        {
                            ...
                            {
                                case "User not found.":
                                    AddLog("Authentication failed. User not found.");
                                    return UpdateStatus.UserNotFound;
                                case "Incorrect password.":
                                    AddLog("Authentication failed. Incorrect password.");
                                    return UpdateStatus.InvalidCredentials;
                                case "Hostname not found.":
                                    AddLog("Hostname not found.");
                                    return UpdateStatus.InvalidHostname;
                                case "User does not have permission for the specified hostname.":
                                    AddLog("User does not have permission for the specified hostname.");
                                    return UpdateStatus.Unauthorized;
                                default:
                                    AddLog("Error from server: " + error);
                                    return UpdateStatus.UpdateFailed;
                            }
                        }
